...
const PRICE_TABLE_NAME = 'closing_price';
...
class Database {
    ...
    storeClosingPrice(stock, price) {
        checkArgTypes(arguments, ['string', 'number']);

        return this.db(PRICE_TABLE_NAME).insert({
            stock,
            time: this._storeCurrentTime(),
            price: this._storePrice(price)
        });
    ...
    getClosingPrice(stock) {
        ...
        return this.db(PRICE_TABLE_NAME).where('stock', stock).orderBy('time', 'desc').limit(1)
            .then((prices) => {
                if (prices.length == 0) {
                    throw new Error('no prices found');
                }
                return Promise.resolve(this._loadPrice(prices[0].price));
            });
    }
